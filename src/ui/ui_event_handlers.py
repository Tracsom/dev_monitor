...
class UIEventHandlers:
    ...
    def __init__(self, scheduler: Scheduler, parent: Tk, refresh_callback: Callable):
        """
    ...
    def _on_device_added(self, device_name: str) -> None:
        ...
        DialogManager.show_success(self.parent, "Device added", f"Added device {device_name}")
        self.refresh_callback()

    def _on_device_add_failed(self, device_name: str) -> None:
        """Handle failed device add."""
        DialogManager.show_error(self.parent, "Add failed", f"Failed to add device: {device_name}")

    def _on_device_removed(self, device_name: str) -> None:
        """Handle successful device removal."""
        DialogManager.show_success(self.parent, "Device removed", f"Removed device: {device_name}")
        self.refresh_callback()

    def _on_device_remove_failed(self, device_name: str) -> None:
        """Handle failed device removal."""
        DialogManager.show_error(self.parent, "Remove failed", f"Failed to remove device: {device_name}")
