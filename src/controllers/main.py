...
class MainController:
    ...
    def _setup_event_handlers(self) -> None:
        ...
        if self.scheduler:
            ...
            self.scheduler.subscribe("remove_device", self.handle_remove_device)
            self.scheduler.subscribe("check_all_devices", self.handle_check_all_devices)
            self.scheduler.subscribe("get_devices", self.handle_get_devices)
            logger.debug("Event handlers registered")

    def handle_add_device(self, name: str, ip_address: str, port: int = 80, timeout: int = 5) -> bool:
        """
        ...
        """
        result = self.device_service.add_device(name, ip_address, port, timeout)
        self.scheduler.publish("device_added" if result else "device_add_failed", name)
        return result
    ...
    def handle_remove_device(self, device_name: str) -> bool:
        ...
        self.scheduler.publish("device_removed" if result else "device_remove_failed", device_name)
        return result
    ...
    def __repr__(self) -> str:
        return f"MainController(scheduler={self.scheduler!r}, devices={len(self.device_service.devices)})"